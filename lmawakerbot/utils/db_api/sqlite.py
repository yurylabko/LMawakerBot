...
from ...types import Chat, Registration


class Database:
    def __init__(self, path_to_db: str = "main.db") -> None:
        self.__path_to_db = path_to_db
        from .migration.sqlite import update_database

        update_database(self)

    @property
    def connection(self):
        return sq.connect(self.__path_to_db)
    ...
    def add_or_update_chat(self, chat: Chat):
        sql_add = """
        insert into chats(id, name, is_active, created_at) VALUES(?, ?, ?, strftime('%s','now'))
        """
        sql_update = """
        update chats set name = ?, is_active = ?,  updated_at = strftime('%s','now') where id = ?
        """
    ...
    def add_or_update_user(self, reg: Registration):
        sql_add = """
        insert into users(chat_id, id, name, alias, phone_number, messengers, is_active, created_at) VALUES(?, ?, ?, ?, ?, ?, ?, strftime('%s','now'))
        """
        sql_update = """
        update users set name = ?, alias = ?, phone_number = ?, messengers = ?, is_active = ?,  updated_at = strftime('%s','now') where chat_id = ? and id = ?
        """
    ...
    def is_exists_user(self, chat_id: int, user_id: int):
        sql = """
        select 1 from users where chat_id = ? and id = ?
        """
        return self.execute(
            sql,
            (
    ...
    def is_exists_name(self, chat_id: int, user_name: str):
        sql = """
        select 1 from users where chat_id = ? and user_name = ?
        """
        return self.execute(
            sql,
            (
    ...
    def show_registrations(self, user_id: int) -> List[Chat]:
        sql = """
        SELECT id, name FROM Chats where id in (select chat_id from Users where id = ?)
        """
